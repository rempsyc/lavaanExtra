...
<!-- 2 x saves time, maybe rephrase the second with? Some suggestions: 
- efficentize the workflow
- optimizie ...
- streamline
- Cut down on time-->

I provide a simple Confirmatory Factor Analysis (CFA) example below,
where the latent variables `visual`, `textual`, and `speed` are defined
by items 1 to 9. We can then use the `cat()` function on the resulting
object (of type character) to read it in the traditional way and make
sure we have not made any mistake.
...
<!-- While the example you describe is pretty much the prototypcial example for SEM -->
<!-- still it is hard to follow if you have never seen the dataset before. -->
<!-- What is ageyr? What does it mean, what do we try to model? Maybe give one -->
<!-- paragraph (including the reference for the data) in which you highlight -->
<!-- what is being modelled here? -->
...
<!-- Popular is maybe not the best scientific adjective, how would
you quantify this if asked? Also 2 x times popular in one sentence,
I would encourage to change the first adjective. Do not get me wrong
I think this function is so awesome, comment just relates to phrasing-->

The most popular {lavaanExtra} function for tables is `nice_fit()`,
which extracts only some of the most popular fit indices and organize
them such that it is easy to compare models. There is an option to
...
<!-- A huge compliment for the visualizations / plots. I know from first hand  -->
<!-- experience how much work and hassle is involved in creating these plots and  -->
<!-- the two functions seem to do an excellent job. -->
