...
The `lavaanExtra` package is an R package providing convenience
functions for structural equation modeling (SEM) with lavaan. It affords
an alternative, vector-based syntax to lavaan, as well as other
convenience functions such as naming paths and defining indirect links
automatically. The package is built using R 3.5+ and follows standard R
...
installed during setup (install as needed per PR) - The lavaanExtra
package itself (built and installed) - Verified functionality of core
functions and reprex
...
box  
- ⚠️ Function-specific packages (lavaanPlot, tidySEM, etc.) need to be
...
Expected results: - ~50+ tests processed across multiple test files -
Most tests should pass - Some snapshot failures due to minor precision
...
Expected results: - Style warnings may be found - focus on new code
adhering to style guidelines - Package is functional despite style
warnings
...
**SEM Syntax Generation**:
[`write_lavaan()`](https://lavaanExtra.remi-theriault.com/reference/write_lavaan.md)
(vector-based lavaan syntax) **Model Fitting**:
[`nice_fit()`](https://lavaanExtra.remi-theriault.com/reference/nice_fit.md)
(extract fit indices with formatting) **Visualization**:
[`nice_lavaanPlot()`](https://lavaanExtra.remi-theriault.com/reference/nice_lavaanPlot.md),
[`nice_tidySEM()`](https://lavaanExtra.remi-theriault.com/reference/nice_tidySEM.md),
[`cfa_fit_plot()`](https://lavaanExtra.remi-theriault.com/reference/cfa_fit_plot.md)
**Data Extraction**:
...
[`nice_modindices()`](https://lavaanExtra.remi-theriault.com/reference/nice_modindices.md)
(modification indices with labels)
...
**Key suggested packages**: rempsyc, flextable, lavaanPlot, tidySEM,
DiagrammeRsvg, rsvg, png, webshot
...
The lavaanExtra package follows this versioning pattern: - **Major
releases** (CRAN submissions): Two decimals (e.g., 0.2.1, 0.2.0,
0.1.9) - **Development versions** (between CRAN releases): Three
decimals (e.g., 0.2.1.1, 0.2.0.3, 0.2.0.2)

### When to Bump Versions

**ALWAYS** bump the version for ANY change that affects the package: 1.
**Bug fixes**: Increment the third decimal (0.2.1 → 0.2.1.1) 2. **New
features**: Increment the third decimal (0.2.1 → 0.2.1.1) 3. **Breaking
changes**: Increment the second decimal (0.2.1 → 0.3.0) - rare 4.
...
work**: Bump version from 0.2.1 → 0.2.1.1 and update NEWS.md 2. **First
commit**: Contains version bump + your first set of changes 3.
**Subsequent commits**: Use the same version (0.2.1.1) for any
...
0.2.1.1 and add new feature X” - Commit 2: “Fix bug in feature X (still
version 0.2.1.1)” - Commit 3: “Update documentation for feature X (still
version 0.2.1.1)” - All commits use version 0.2.1.1, NEWS.md updated
once in commit 1

**NEVER do this** (incorrect approach): - Commit 1: “Add feature X, bump
to 0.2.1.1” - Commit 2: “Fix bug in feature X, bump to 0.2.1.2” ❌
WRONG - Commit 3: “Update docs, bump to 0.2.1.3” ❌ WRONG
...
1.  **For first change after a major release** (e.g., after 0.2.1 was
    released):
...
2.  **For subsequent development changes** (when 0.2.1.1 already
    exists):
...
3.  **Change description guidelines**:
    ...
    - Examples:
      - [`nice_fit()`](https://lavaanExtra.remi-theriault.com/reference/nice_fit.md):
        fix bug with categorical variables leading to an error (#37)
      - [`write_lavaan()`](https://lavaanExtra.remi-theriault.com/reference/write_lavaan.md):
        add new `threshold` argument for threshold operators  
      - [`nice_tidySEM()`](https://lavaanExtra.remi-theriault.com/reference/nice_tidySEM.md)
        now requires `tmvnsim` package for tidySEM compatibility
...
for all 15+ suggested packages at once.
