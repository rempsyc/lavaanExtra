...
data.frame(Drop = x[1:3], Omega = sapply(
  x[1:3],
  USE.NAMES = FALSE, function(i) compRelSEM(fit1, omit.indicators = i)
))
data.frame(Drop = x[4:6], Omega = sapply(
  x[4:6],
  USE.NAMES = FALSE, function(i) compRelSEM(fit2, omit.indicators = i)
))
data.frame(Drop = x[7:9], Omega = sapply(
  x[7:9],
  USE.NAMES = FALSE, function(i) compRelSEM(fit3, omit.indicators = i)
))
```
